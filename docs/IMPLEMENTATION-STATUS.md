...
| 0021 | Hub Read & Management API | Accepted | 🚧 | **Phase A+B done**: read endpoints (summary/branches/commits/commit) via `internal/hub` projection (H2/H8), `read`-gated, ETag/304, cursor pagination, `X-VARA-API`/`X-Request-ID`; `vara serve --hub` same-origin static serving (SPA fallback, strict route precedence H3) + httpOnly/SameSite=Strict cookie sessions. **Pending**: Phase C (Hub UI) |
