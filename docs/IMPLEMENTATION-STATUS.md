...
| 0021 | Hub Read & Management API | Accepted | ✅ | Read endpoints (summary/branches/commits/commit) via `internal/hub` projection (H2/H8), `read`-gated, ETag/304, cursor pagination, `X-VARA-API`/`X-Request-ID`; `vara serve --hub` same-origin static serving + httpOnly/SameSite=Strict cookie sessions; `web/` — minimal framework-free Hub UI (login/dashboard/overview/history/branches/settings) |
